...
list_upgradable() {
    echo $(apt list --upgradable 2>&1 | \
    grep -Ev "Listing...|apt does not have a stable CLI interface" | \
    cut -d/ -f1 | \
    awk '{$1=$1};1' | \
    tr '\r\n' ' ')
}
